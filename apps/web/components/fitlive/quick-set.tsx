...
import {parseSetSpeech,parseSetCorrection} from "@/lib/gym-speech";
...
export function QuickSet({workout,state,draft,owner,onAdd,act,onCorrection}:{workout:ReturnType<typeof plan>;state:State;draft:SetLog[];owner:string;onAdd:(set:SetLog)=>void;act:(c:Command,close?:boolean)=>Promise<boolean>;onCorrection:(patch:{load?:number;reps?:number})=>void}){
...
function SetRow({exercise,number,state,owner,onAdd,onCorrection}:{exercise:ReturnType<typeof plan>[number];number:number;state:State;owner:string;onAdd:(set:SetLog)=>void;onCorrection:(patch:{load?:number;reps?:number})=>void}){
 ...
 const [load,setLoad]=useState<number|null>(previous?.load??(bodyweight?0:null)),[reps,setReps]=useState(previous?.reps??10),[rpe,setRpe]=useState<number|undefined>(),[typed,setTyped]=useState(false),[voice,setVoice]=useState(false),[voiceNote,setVoiceNote]=useState(""),[taps,setTaps]=useState(0),[started]=useState(()=>performance.now());
 ...
 {!bodyweight&&<SetStepper label={`Load · ${unit}`} value={shown} min={.5} max={unit==="kg"?500:1100} step={step} onChange={n=>{setTyped(true);setTaps(taps+1);setLoad(toKg(n,unit));}}/>}
 <SetStepper label="Reps" value={reps} min={1} max={100} step={1} onChange={n=>{setTyped(true);setTaps(taps+1);setReps(Math.round(n));}}/>
