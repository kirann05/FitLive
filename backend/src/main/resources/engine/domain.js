...
var FitLiveDomain = (() => {
  ...
  function progression(s, exercise, base = 20, now = /* @__PURE__ */ new Date()) {
    const recent = s.workouts.filter((w) => w.sets.some((x) => x.exercise === exercise)).sort((a, b) => b.date.localeCompare(a.date));
    const last = recent[0];
    if (!last)
      return {
        load: base,
    ...
    if (last.effort === "Too hard" || recent.length >= 2 && recent.slice(0, 2).every(
      (w) => w.sets.filter((x) => x.exercise === exercise).some((x) => x.reps < 8)
    ))
      return {
        load: Math.round(load * 0.9 * 2) / 2,
        reason: "Recent difficulty suggests a small reduction before progressing."
      };
    if (last.status === "completed" && sets.length >= 3 && sets.every((x) => x.reps >= 12 && x.rpe <= 8))
      return {
        load: load + 2.5,
        reason: "All three working sets reached 12 reps at RPE 8 or less. Add 2.5 kg."
      };
    return {
      load,
    ...
    if (s.program?.sessions.length) {
      ...
      return session.exercises.map((x) => {
        const previous = [...s.workouts].sort((a, b) => b.date.localeCompare(a.date)).find(
          (w) => w.status === "completed" && w.sets.some((a) => a.exercise === x.name)
        );
        const sets = previous?.sets.filter((a) => a.exercise === x.name) ?? [];
        const lastLoad = sets.length ? Math.max(...sets.map((a) => a.load)) : x.load;
        const advance = sets.length >= x.sets && sets.every((a) => a.reps >= x.maxReps && a.rpe <= 8);
